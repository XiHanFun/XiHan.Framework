...
{
    private readonly ITelegramConfigStore _configStore;

    /// <summary>
    /// 创建提供者
    /// </summary>
    public TelegramBotProvider(ITelegramConfigStore configStore)
    {
    ...
    public async Task<BotResult> SendAsync(BotMessage message, BotContext context)
    {
        ...
        {
            return BotResult.BadRequest("Telegram provider is not configured or disabled.", Name);
        }
    ...
    private static ChatId ResolveChatId(BotMessage message, TelegramOptions options)
    {
        ...
        if (BotMessageHelper.TryGetData(message, TelegramMessageDataKeys.TelegramChatId, out string? overrideChatId)
    ...
    {
        if (BotMessageHelper.TryGetData(message, TelegramMessageDataKeys.TelegramParseMode, out string? parseModeValue)
