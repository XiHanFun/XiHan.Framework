...
/// 平台归属用户（TenantId=0）或跨租户成员（归属租户 A、当前活动在租户 B）写自己的这些行是完全合法的。
/// </para>
/// <para>
/// 作用域内仓储对象式写（Update/Delete/软删/恢复家族）的<b>预读同时忽略租户过滤</b>：
/// 自有行带的是归属租户戳，带租户过滤的预读会把它判为「不存在」；条件写（表达式更新/谓词删除）则不追加当前租户 Where。
/// </para>
/// <para>
