...
// FileName:ExceptionLogQueueWorker
// Guid:19e9a215-8b3d-4f52-8d77-574d47b4c0cd
// Author:zhaifanhua
...
public class ExceptionLogQueueWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var batch = new List<ExceptionLogRecord>(_options.BatchSize);
    ...
    private async Task FlushAsync(List<ExceptionLogRecord> batch, CancellationToken cancellationToken)
    {
        if (batch.Count == 0)
        {
            return;
        ...
        try
        {
            ...
            var writer = scope.ServiceProvider.GetService<IExceptionLogWriter>();
            if (writer is null)
            {
                return;
            }
        ...
        {
            _logger.LogWarning(ex, "异常日志队列消费失败");
        }
    }
}
