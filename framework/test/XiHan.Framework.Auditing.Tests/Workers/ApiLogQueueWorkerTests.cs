...
/// 接口日志队列消费者测试
/// </summary>
public class ApiLogQueueWorkerTests
{
    /// <summary>
    ...
    private static ServiceProvider BuildProvider(IApiLogWriter writer)
    {
        var services = new ServiceCollection();
        services.AddScoped<IApiLogWriter>(_ => writer);
        return services.BuildServiceProvider();
    }

    private static ApiLogQueueWorker CreateWorker(
        ScriptedLogQueue<ApiLogRecord> queue,
        IServiceProvider provider,
        XiHanAuditingLogQueueOptions options)
    {
        return new ApiLogQueueWorker(
            queue,
            provider.GetRequiredService<IServiceScopeFactory>(),
            Microsoft.Extensions.Options.Options.Create(options),
            NullLogger<ApiLogQueueWorker>.Instance);
    }
}
