...
/// </summary>
/// <remarks>
/// 异常日志是排障最后一道线索，「停止时冲刷剩余批次」这条尤其不能坏——
/// 进程正常下线时压在批次里的异常记录必须落库，否则故障现场随进程一起消失。
/// </remarks>
public class ExceptionLogQueueWorkerTests
{
    /// <summary>
    ...
    private static ServiceProvider BuildProvider(IExceptionLogWriter writer)
    {
        var services = new ServiceCollection();
        services.AddScoped<IExceptionLogWriter>(_ => writer);
        return services.BuildServiceProvider();
    }

    private static ExceptionLogQueueWorker CreateWorker(
        ScriptedLogQueue<ExceptionLogRecord> queue,
        IServiceProvider provider,
        XiHanAuditingLogQueueOptions options)
    {
        return new ExceptionLogQueueWorker(
            queue,
            provider.GetRequiredService<IServiceScopeFactory>(),
            Microsoft.Extensions.Options.Options.Create(options),
            NullLogger<ExceptionLogQueueWorker>.Instance);
    }
}
