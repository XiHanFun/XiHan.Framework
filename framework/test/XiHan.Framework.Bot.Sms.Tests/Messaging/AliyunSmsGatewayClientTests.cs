...
/// <summary>
/// <see cref="AliyunSmsGatewayClient"/> 阿里云短信网关客户端测试
/// </summary>
/// <remarks>
/// 只覆盖到达阿里云 SDK 之前的本地前置校验：入参非空、取消令牌、模板映射解析、模板参数解析。
/// 这些分支全部在 <c>_client.SendSmsAsync</c> 之前短路返回或抛出，因此整组用例零网络请求。
/// 真正的投递成功路径需要真实凭据，用 Assert.Skip 显式标记。
/// </remarks>
public class AliyunSmsGatewayClientTests
{
    private const string TemplateMapJson = """{"auth-sms-login-code":{"templateCode":"SMS_123456","paramOrder":["code"]}}""";

    /// <summary>
    /// 服务商类型固定为阿里云，供解析器与日志归因
    /// </summary>
    [Fact]
    public void Provider_IsAliyun()
    {
    ...
    /// <summary>
    /// 模板映射为空时任何模板码都拒绝发送，不会带着空模板码打到阿里云
    /// </summary>
    [Fact]
    public async Task SendAsync_WhenTemplateMapEmpty_Throws()
    {
        var client = new AliyunSmsGatewayClient("test-access-key-id", "test-access-key-secret", "曦寒科技", null);
    ...
    /// <summary>
    /// 真实投递成功路径需要可用的阿里云凭据与网络，CI 不具备
    /// </summary>
    [Fact]
    public void SendAsync_SuccessPath_RequiresRealAliyunCredentials()
    {
        Assert.Skip("需要真实阿里云短信凭据与网络，CI 不具备");
    }

    /// <summary>
    /// 构造一个带模板映射的阿里云客户端（仅本地装配，不触网）
    /// </summary>
    /// <returns>阿里云短信网关客户端</returns>
    private static AliyunSmsGatewayClient CreateClient()
    {
        return new AliyunSmsGatewayClient("test-access-key-id", "test-access-key-secret", "曦寒科技", TemplateMapJson);
    }
