...
默认的 `DefaultGrayRuleRepository` 是最多保存 10000 条规则的单例进程内仓储（满载时 `AddRule` 抛 `InvalidOperationException`）：规则重启即丢、多实例各存各的、没有任何管理入口（`AddRule` / `RemoveRule` / `Clear` 不在 `IGrayRuleRepository` 接口上）。
