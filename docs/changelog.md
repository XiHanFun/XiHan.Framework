...
- **新增** NuGet 发布自动化（publish.yml：手动触发或 v* 标签，测试通过后推送 nuget.org），本机脚本保留为备选
- **修复** 分页执行器未应用 Skip/Take，分页接口返回整表匹配记录而非当前页（Domain.Shared，补盲测试暴露）
...
- **修复** 钉住 SQLitePCLRaw 2.1.13，修复 SqlSugarCore 传递依赖高危漏洞 CVE-2025-6965；高危/严重漏洞进构建门禁（NU1903/NU1904 升级为错误）
- **优化** 测试全量迁移 Microsoft.Testing.Platform 模式（.NET 10 下旧 VSTest 路径不可用），统一测试栈并移除 coverlet 残留
- **优化** 构建警告清零；NuGet 包接入 SourceLink；SDK 锁定 10.0.3xx 功能带；文档依赖版本对齐实际引用
- **升级** 升级依赖，发布 v3.13.1
...
- **调整** 文档迁入本仓 `docs/` 独立成站，更新日志归口文档站
- **调整** 国内镜像由 AtomGit 迁到 GitCode
- **优化** 383 处 `<inheritdoc />` 替换为完整 XML 文档注释
- **升级** 升级依赖，发布 v3.11.0
