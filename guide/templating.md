来源：https://framework.docs.xihanfun.com/guide/templating
...
把 Scriban 语法（`{{ name | string.upcase }}`、`{{ user.Name }}`）丢给 `ITemplateService`，得到的是原样输出，而且不报错。
:::
...
| 语法 | 说明 |
| --- | --- |
| `{{变量名}}` | 字符串精确替换，值取 `ToString()`，null 变空串 |
| `{{if 条件}}…{{else}}…{{endif}}` | 条件仅支持 `==`、`!=` 和「变量真值判断」 |
| `{{for 项 in 集合}}…{{endfor}}` | 集合必须是 `IEnumerable` |

渲染顺序固定为：**先条件 → 再循环 → 最后变量替换**。

::: warning 占位符里不能有空格
替换用的占位串是把键名直接夹在两对花括号之间拼出来的，`{{ Name }}`（带空格）匹配不上，会原样留在输出里。这是「模板没生效」最常见的原因。
:::

其它硬限制，都源于它是正则/字符串替换实现：

- **没有成员访问**：`{{user.Name}}` 不会解析对象，除非变量键名字面上就叫 `user.Name`（见下面上下文构建器的写法）。
- **循环项只能整体输出**：循环体里 `{{item}}` 输出的是元素的 `ToString()`，写 `{{item.Title}}` 拿不到属性。
- **不支持嵌套**：`if` / `for` 的匹配是非贪婪正则，内层的 `{{endif}}` / `{{endfor}}` 会被外层先吃掉，嵌套写法结果错乱。
...
| 调用 | 实际校验什么 |
| --- | --- |
| `ITemplateService.ValidateTemplate(source)` | 走 String 引擎：只数 `{{` / `}}`、`if`/`endif`、`for`/`endfor` 是否成对 |
...
- 子模板的 `extends` / `block` 用宽松正则解析，写 `{{ block x }}` 或 `{{- block x -}}` 都认。
- **布局里的块标记必须精确写成 `{{- block 名字 -}}` 和 `{{- endblock -}}`**（带短横线），合并时是按这两个字面串查找的，写成 `{{ block x }}` 不会被替换。
...
| 现象 | 原因 |
| --- | --- |
| 模板原样输出，占位符没被替换 | 占位符里带了空格；简单引擎只匹配严格的 `{{键}}` |
| Scriban 过滤器 / 管道写法不生效 | `ITemplateService` 走的是 String 引擎，不解析 Scriban |
| `{{user.Name}}` 没被替换 | 简单引擎无成员访问；用 `AddObject(obj, "user")` 造出 `user.Name` 键，或改用 Scriban |
...
| 布局的 block 没被替换 | 布局里必须写成 `{{- block x -}}` / `{{- endblock -}}` |
