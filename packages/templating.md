来源：https://framework.docs.xihanfun.com/packages/templating
...
- `DefaultTemplateEngine` 只支持三类占位语法（用正则/字符串替换实现，见下），**不认识 Scriban 的过滤器、函数、成员访问等**：
  - 变量占位：`{{变量名}}`
  - 条件：`{{if 条件}}...{{else}}...{{endif}}`，条件仅支持 `==` / `!=` / 变量存在性
...
`Simple` 命名空间是一套完全独立、**不经过 DI / `ITemplateEngineRegistry` / `ITemplateService`** 的静态占位替换工具，语法与 `DefaultTemplateEngine` 类似（`{{变量}}` / `{{if}}...{{else}}...{{endif}}` / `{{for x in list}}...{{endfor}}`），但代码实现互相独立、不共享缓存：
...
- **占位语法差异**：简单引擎的条件仅支持 `==` / `!=` / 变量存在性判断，循环仅支持 `{{for x in list}}`；复杂逻辑请用 Scriban。
